...
linking it against all the necessary libraries.

TODO: describe an appropriate setup.py.
...
The ``setup.py`` script described in the previous example can be used for this
...
previous examples.
    ...
    - The :directive:`%Import` directive has been added to specify that we are
      ...
      ``QtWidgets/QtWidgetsmod.sip``.  This file is part of PyQt5.
      ``setup.py`` will determine the file's exact location.
    ...
    - The :directive:`%If` directive has been added to specify that everything
      ...
      v5.2 and later.  ``Qt_5_2_0`` is a *tag* defined in ``QtCoremod.sip``
...
.. [#] Actually in ``versions.sip``.  PyQt5 uses the :directive:`%Include`
